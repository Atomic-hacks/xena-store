    ...
    <div className="space-y-4">
      <section className={`p-5 md:p-6 ${glassStyles.card}`}>
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-neutral-900">{customer.fullName}</h2>
            <p className="text-sm text-neutral-600">{customer.phone}</p>
            {customer.email ? <p className="text-sm text-neutral-600">{customer.email}</p> : null}
            {customer.defaultLocation ? (
              <p className="mt-1 text-sm text-neutral-500">{customer.defaultLocation}</p>
            ) : null}
          </div>
          <button
            type="button"
            ...
            className="rounded-full bg-neutral-100 px-3 py-1 text-xs text-neutral-700 hover:bg-neutral-200 disabled:opacity-60"
          >
